...
const TabIcon = ({ type, color }: { type: 'journeys' | 'map' | 'search' | 'profile'; color: ColorValue }) => <View style={[styles.icon, { borderColor: color }]}>
  ...
  {type === 'search' ? <><View style={[styles.lens, { borderColor: color }]} /><View style={[styles.handle, { backgroundColor: color }]} /></> : null}
...
export default function TabsLayout() {
  return (
    <Tabs screenOptions={{ headerShown: false, tabBarActiveTintColor: colors.ink, tabBarInactiveTintColor: '#9A9891', tabBarStyle: { backgroundColor: colors.tab, borderTopColor: colors.line, height: 84, paddingTop: 8 }, tabBarLabelStyle: { fontSize: 11, fontWeight: '600' } }}>
      ...
      <Tabs.Screen name="search" options={{ title: 'Search', tabBarIcon: ({ color }) => <TabIcon type="search" color={color} /> }} />
      <Tabs.Screen name="profile" options={{ title: 'Profile', tabBarIcon: ({ color }) => <TabIcon type="profile" color={color} /> }} />
    </Tabs>
  );
}

const styles = StyleSheet.create({ icon: { width: 22, height: 22, alignItems: 'center', justifyContent: 'center' }, dot: { width: 10, height: 10, borderRadius: 5 }, pin: { width: 14, height: 14, borderWidth: 2, borderRadius: 8, transform: [{ rotate: '45deg' }] }, pinDot: { position: 'absolute', width: 4, height: 4, borderRadius: 2 }, lens: { position: 'absolute', left: 2, top: 2, width: 14, height: 14, borderWidth: 2, borderRadius: 8 }, handle: { position: 'absolute', width: 8, height: 2, right: 1, bottom: 3, borderRadius: 1, transform: [{ rotate: '45deg' }] }, head: { position: 'absolute', top: 3, width: 7, height: 7, borderRadius: 4 }, shoulders: { position: 'absolute', bottom: 2, width: 16, height: 9, borderWidth: 2, borderBottomWidth: 0, borderTopLeftRadius: 9, borderTopRightRadius: 9 } });
